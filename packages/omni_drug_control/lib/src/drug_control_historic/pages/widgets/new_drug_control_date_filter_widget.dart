...
class NewDrugControlDateFilterWidget extends StatefulWidget {
  final DatePickerController dateController;

  const NewDrugControlDateFilterWidget({
    Key? key,
    required this.dateController,
  }) : super(key: key);

  @override
  _NewDrugControlDateFilterWidgetState createState() => _NewDrugControlDateFilterWidgetState();
}

class _NewDrugControlDateFilterWidgetState extends State<NewDrugControlDateFilterWidget> {
  ...
  Widget _buildDatePickerWidget() {
    return ScopedBuilder<MedicineHistoricStore, DioError, List<DateTime>>(
      ...
      onState: (_, activeDates) {
        return TripleBuilder<MedicineHistoricDateFilterStore, DioError, DateTime>(
          store: store,
          builder: (_, triple) {
