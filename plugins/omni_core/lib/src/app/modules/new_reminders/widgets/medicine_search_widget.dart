...
class MedicineSearchWidget extends StatefulWidget {
  ...
  @override
  _NewDrugControlMedicinesWidgetState createState() => _NewDrugControlMedicinesWidgetState();
}

class _NewDrugControlMedicinesWidgetState extends State<MedicineSearchWidget> {
    ...
    scrollController.addListener(() {
      if (widget.useCustomMedication) {
        if (scrollController.offset == scrollController.position.maxScrollExtent &&
            store.customMedicines.results!.length != store.customMedicines.count) {
          store.params.limit = (int.parse(store.params.limit!) + 10).toString();
          store.getMedicines(store.params, widget.useCustomMedication);
        }
      } else {
        if (scrollController.offset == scrollController.position.maxScrollExtent &&
            store.generalMedicines.results!.length != store.generalMedicines.count) {
          store.params.limit = (int.parse(store.params.limit!) + 10).toString();
          store.getMedicines(store.params, widget.useCustomMedication);
        }
      }
    ...
    return TripleBuilder<NewDrugControlMedicineStore, DioError, List>(
      ...
      builder: (_, triple) {
        return TextFieldWidget(
          ...
          suffixIcon:
              triple.isLoading ? const CircularProgressIndicator.adaptive() : Icon(Icons.keyboard_arrow_down_rounded),
          onTap: () async {
            ...
            child: Column(
              ...
              children: [
                BottomSheetHeaderWidget(
                  ...
                  searchPlaceholder: DrugControlLabels.newDrugControlMedicineSearchPlaceholder,
                  showSearch: true,
                    ...
                    child: SafeArea(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            DrugControlLabels.newDrugControlMedicineSearchLoading,
                ...
                if (widget.useCustomMedication
                  ...
                  return const SafeArea(
                    child: Padding(
                      ...
                      child: EmptyWidget(
                        message: DrugControlLabels.newDrugControlMedicineListEmpty,
                      ),
                    ),
                  );
                }
